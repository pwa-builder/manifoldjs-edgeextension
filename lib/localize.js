...
function getResources(extManifest, localesList) {
    ...
    if (shouldLocalize(extManifest)) {          
        ...
        var resourceString = resourceTemplate.replace(resourceKey, defaultLocale);
...
function makePriConfig(priConfigFilePath, extManifest) {
    ...
    .then(function (toolPath) {
        ...
        exec(cmdLine, { maxBuffer: 1024*1024 }, function (err, stdout, stderr) {             
            if (err) {
                var errmsg = stdout.substring(164, 480);
                return deferred.reject(new Error(errmsg ? errmsg : 'makePriFiles failed.'));
            }
...
function makePriFiles(manifestDir, extManifest) {
    ...
    .then(function (toolPath) {
        ...
        var deferred = Q.defer();
        exec(cmdLine, { maxBuffer: 128*1024 }, function (err, stdout, stderr) {             
            if (err) {
                var errmsg = stdout.substring(164, 480);
                return deferred.reject(new Error(errmsg ? errmsg : 'makePriFiles failed.'));
            }
